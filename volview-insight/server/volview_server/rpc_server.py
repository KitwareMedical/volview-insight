...
class RpcServer:
    ...
    async def _on_connect(self, sid: str, environ: dict):
        ...
        print("_on_connect: here is the clients dictionary (keys are session ids):")
        print(self.clients)
        # add to room based on client_id. We think the server is not in the
        # right room with the client. It needs to be in the same room to ask for
        # the data. The client doesn't even know.

        # idea is a client can talk to a server, it can always. But the room
        # idea is better for servers so it can talk to multiple clients
        self.sio.enter_room(sid, client_id) # sid session id of client, and the room name wich is the client id. server enters ar oom with the client
